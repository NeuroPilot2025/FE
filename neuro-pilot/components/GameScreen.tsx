...
const GameScreen: React.FC<GameScreenProps> = ({ nickname, selectedBunny, onGameOver }) => {
  ...
  useEffect(() => {
    ...
    const gameTick = (timestamp: number) => {
      ...
      setMissiles(prev => prev.map(m => ({ ...m, y: m.y - MISSILE_SPEED * (deltaTime * 60) })).filter(m => m.y > -MISSILE_HEIGHT));
      ...
      setEnemies(prev => prev.map(enemy => {
        ...
        const remaining = missiles.filter(missile => {
          ...
          if (collided) { newHealth--; hit = true; return false; }
          return true;
        });

        if (hit) {
          setMissiles(remaining);
          const updated = { ...enemy, health: newHealth, isHit: true };
          ...
          if (newHealth <= 0) {
            ...
            setTimeout(() => {
              setEnemies(cur => cur.filter(e => e.id !== updated.id));
              if (isLastStrong) {
            ...
            enemyHitTimeoutRef.current = setTimeout(() => {
              setEnemies(cur => cur.map(e => e.id === enemy.id ? { ...e, isHit: false } : e));
            }, 150);
          }
          return updated;
        }
    ...
    return (
      <div className="w-full h-full flex flex-col items-center justify-center relative bg-[#0c1445]">
        ...
        <div className="z-10 p-8 bg-black/70 rounded-lg text-center">
          <img src={outcomeImageSrc} alt={altText} className="max-w-sm mb-6 pixelated" />
    ...
    <div className="relative overflow-hidden shadow-2xl border-4 border-blue-800/70" style={{ width: GAME_WIDTH, height: GAME_HEIGHT }}>
      ...
      <div className="absolute inset-0 z-10">
        ...
        {missiles.map(missile => (
          <MissileComponent key={missile.id} x={missile.x} y={missile.y} selectedBunny={selectedBunny} />
        ))}
